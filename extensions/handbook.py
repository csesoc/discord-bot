...
class Handbook(commands.Cog):
    ...
    async def courseinfo(self, ctx, course_code, *options):
        ...
        overview_text = BeautifulSoup(data['description'].split('\n')[0], "html.parser").get_text()
        course_info.add_field(name="Overview", value=overview_text, inline=False)

        if data['enrolment_requirements']:
            enrolment_requirements_text = re.sub('[A-Z]{4}[0-9]{4}', f'[\g<0>]({HANDBOOK_URL}\g<0>)', data['enrolment_requirements'])
        else:
            enrolment_requirements_text = "None"
        course_info.add_field(name="Enrolment Requirements", value=enrolment_requirements_text)
        ...
        if data['equivalent_courses']:
            equivalent_courses_text = ', '.join(f"[{course}]({HANDBOOK_URL}{course})" for course in data['equivalent_courses'])
        else:
        ...
        if data['exclusion_courses']:
            exclusion_courses_text = ', '.join(f"[{course}]({HANDBOOK_URL}{course})" for course in data['exclusion_courses'])
        else:
            exclusion_courses_text = "None"
        course_info.add_field(name="Exclusion Courses", value=exclusion_courses_text)

        course_outline_text = f"[{course_code} Course Outline]({data['course_outline_url']})"
