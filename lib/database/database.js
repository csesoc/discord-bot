...
class DBuser {
    ...
    async user_leave(userid) {
        ...
        try{
        await client.query("BEGIN");
        var values = [time, true ,userid];
        var query = `UPDATE users  SET leavedate = $1 , userleft = $2 where userid = $3`;
    ...
    async user_join(userid) {
        ...
        if(result.rows.length != 0) {
            var query = `UPDATE users SET joindate=$1, userleft=$2 where userid=$3`;
            values = [time, false, userid];
            await client.query(query,values);

        }else {
            var query = `INSERT INTO users (USERID, JOINDATE, LEAVEDATE, USERLEFT) VALUES ($1,$2,$3,$4)`;
            var values = [userid, time, null, false];
            await client.query(query,values);
        }
        await client.query("COMMIT");
        }
    ...
    async add_user_role(userid, role) {
        ...
        var query = `SELECT max(rid) from user_roles`;
        var result = await client.query(query);
        
        var count = result.rows[0]['max'] + 1;
    ...
    async add_user_permission(userid, permission) {
        ...
        var query = `SELECT max(pid) from user_permissions`;
        var result = await client.query(query);
        var count = result.rows[0]['max'] + 1;
        //console.log(result.rows);
        query = `INSERT INTO user_permissions (PID, USERID, PERMISSION) VALUES ($1,$2,$3)`;
        var values = [count, userid, permission];
        var result = await client.query(query, values);
        await client.query("COMMIT");
        }
        catch (ex)
    ...
    async user_join_channel(userid, channel) {
        ...
        var query = `SELECT max(cid) from user_channels`;
        var result = await client.query(query);
        var count = result.rows[0]['max'] + 1;
