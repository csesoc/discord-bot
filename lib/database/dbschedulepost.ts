...
export class DBSchedulePost {
    ...
    async get_scheduled(datetime: any) {
        ...
        try {
            ...
            const result = await client.query(
                "select * from schedule_post where datetime=$1",
                values,
            );
            await client.query("COMMIT");
    ...
    async get_reminders(reminder: any) {
        ...
        try {
            ...
            const result = await client.query(
                "select * from schedule_post where reminder=$1",
                values,
            );
            await client.query("COMMIT");
    ...
    async get_scheduled_post_id(msg_id: String, send_channel_id: String, datetime: any) {
        ...
        try {
            ...
            const result = await client.query(
                "select * from schedule_post where msg_id=$1 and send_channel_id=$2 and datetime=$3",
                values,
            );
            await client.query("COMMIT");
