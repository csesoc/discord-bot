...
class DBlog {
    ...
    async channel_add(channel_id, channel_name, guild_id) {
        ...
        try {
            ...
            if (result.rows.length == 0) {
                let query = "INSERT INTO channels VALUES ($1,$2,$3)";
                let values = [channel_id, guild_id, channel_name];
    ...
    async channel_delete(channel_id) {
        ...
        try {
            ...
            if (result.rows.length != 0) {
                let query = "delete from message_logs where channel_id = $1";
                let values = [channel_id];
                await client.query(query, values);

                let query = "delete from channels where channel_id = $1";
                let values = [channel_id];
                await client.query(query, values);
            }
    ...
    async message_delete(messageid) {
        ...
        try {
            ...
            if (result.rows.length == 1 && result.rows[0].deleted == 0) {
                let query = "UPDATE message_logs SET deleted=$1 where message_id=$2";
                values = [1, messageid];
                await client.query(query, values);
            } else {
