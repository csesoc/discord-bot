...
class DBSchedulePost {
    constructor() {
        ...
        (async () => {
            var has_table = await this.check_table('schedule_post');
            if (has_table == false) {
    ...
    async create_schedule_post_table() {
        ...
        try {
            ...
            await client.query("BEGIN");
            var query = `CREATE TABLE SCHEDULE_POST (
    ...
    async add_react_role_msg(guild_id, msg_id, init_channel_id, send_channel_id, datetime, reminder) {
        ...
        try{
            ...
            var query = `INSERT INTO schedule_post VALUES (DEFAULT,$1,$2,$3,$4,$5,$6)`;
            var values = [guild_id, msg_id, init_channel_id, send_channel_id, datetime, reminder];
    ...
    async add_reminder(sent_msg_id, scheduled_post_id) {
        ...
        try{
            await client.query("BEGIN");
            var query = `UPDATE schedule_post SET sent_msg_id=$1 WHERE scheduled_post_id=$2;`;
            var values = [sent_msg_id, scheduled_post_id];
            await client.query(query, values);
            await client.query("COMMIT");
        }
        catch (ex)
    ...
    async get_scheduled(datetime) {
        ...
        try {
            ...
            var result = await client.query("select * from schedule_post where datetime=$1",values)
            await client.query("COMMIT");
    ...
    async get_reminders(reminder) {
        ...
        try {
            ...
            var result = await client.query("select * from schedule_post where reminder=$1",values)
            await client.query("COMMIT");
    ...
    async get_scheduled_post_id(msg_id, send_channel_id, datetime) {
        ...
        try {
            ...
            var result = await client.query("select * from schedule_post where msg_id=$1 and send_channel_id=$2 and datetime=$3",values)
            await client.query("COMMIT");
