const { 
    MessageEmbed, Message, CommandInteraction, 
    InteractionReplyOptions, MessageButton, MessageActionRow, 
...
class DiscordScroll {
    ...
    /** @protected */
    _buttons = {
        left: new MessageButton().setCustomId("scrollLeft").setEmoji("⬅️").setStyle("SECONDARY"),
        right: new MessageButton().setCustomId("scrollRight").setEmoji("➡️").setStyle("SECONDARY"),
        delete: new MessageButton().setCustomId("scrollDelete").setEmoji("🚮").setStyle("DANGER"),
    }
    
    /** @protected @type {?MessageEmbed} */
    _embed = null;
    ...
    /**
     * @param {MessageEmbed[]} value The array of Embeds
     */
    set pages(value) {
        ...
        } else if (!(value.every(e => e instanceof MessageEmbed))) {
            throw new TypeError("DiscordScroll.pages expected an array of MessageEmbeds.");
        }
    ...
    get _getButtonRow() {
        ...
        return new MessageActionRow().addComponents(this._buttons.left, this._buttons.right, this._buttons.delete);
    }
    ...
    async _update(buttonInt) {
        this._embed = this.currentPage;
        await buttonInt.update({ embeds: [this.embed], components: [this._getButtonRow] });
    }
