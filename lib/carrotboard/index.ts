...
import { Channel, ChannelType, Client, EmbedBuilder, EmbedData, MessageReaction, User } from "discord.js";
import fs from "fs";
const YAML = require("yaml");

interface LeaderboardOptionsType {
...
export class CarrotboardStorage {
    ...
    async sendCBAlert(messageReaction: MessageReaction, CBID: number, emoji: string) {
        // get the alert channel and check it
        let alertChannel: Channel | null;
        try {
            alertChannel = await this._client.channels.fetch(this.config.alertChannelID) as Channel;
        } catch {
        ...
        let messageContent = message.cleanContent as string;
        ...
        const embedOptions: EmbedData = {
            ...
            color: Number(message.member?.displayHexColor),
            footer: { text: `ID: ${CBID}` },
            url: message.url,
            thumbnail: {
    ...
    async generateLeaderboard({ onlyFirstPage = false, userID = null, emoji = null }: LeaderboardOptionsType) {
        // get the entries
...
class CarrotboardConfig {
    protected _leaderboardID: string;
    protected _permaChannelID: string;
    protected _alertChannelID: string;
    protected _guildID: string;
    protected _carrot: string;
    protected _minimum: number;
    protected _pinMinimum: number;
    ...
    set permaChannelID(value: string) {
        if (typeof value != "string" && typeof value != "bigint") {
    ...
    set alertChannelID(value: string) {
        if (typeof value != "string" && typeof value != "bigint") {
