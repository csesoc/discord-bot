    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                .setName("create")
                .setDescription(
                    "Schedule a message to be sent to a nominated channel at a specific time.",
                )
                ...
                .addStringOption((option) =>
                    option
                        .setName("reminder")
                        .setDescription(
                            "Optional: Send a reminder to users about this who react with ⏰ at YYYY-MM-DD HH:MM",
                        ),
                ),
        )
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addChannelOption((option) =>
                    option
                        .setName("channel")
                        .setDescription(
                            "Select the channel where the message is scheduled to send.",
                        )
                        .setRequired(true),
                )
                .addStringOption((option) =>
                    option
                        .setName("datetime")
                        .setDescription(
                            "Enter the time of the initial scheduled message as YYYY-MM-DD HH:MM",
                        )
                        .setRequired(true),
                ),
        ),
...
async function create_scheduled_post(interaction, msg_id, channel, datetime) {
    ...
    if (user_reminder && user_reminder <= datetime) {
        return await interaction.reply({
            content:
                "Please enter a reminder date that is after the datetime of the scheduled post",
            ephemeral: true,
        });
    }
