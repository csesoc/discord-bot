...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Creates a new role and assigns role to anyone who reacts with given emoji"
    )
  ...
  async execute(interaction) {
    ...
    for (element of emojiList) {
      if (
        !unicode_emoji_regex.test(element) &&
        !custom_emoji_regex.test(element)
      ) {
        return await interaction.reply({
          content:
            "Please enter emojis only separated by commas e.g. emoji,emoji",
          ephemeral: true,
        });
      }
    }
    ...
    try {
      ...
      for (let emoji in roles) {
        await reactRoles.add_react_role_role(
          roles[emoji],
          emoji,
          sentMessage.id
        );
