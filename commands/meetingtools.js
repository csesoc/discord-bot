...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addStringOption(option => option.setName("exclude").setDescription("Exclude certain vc participants, enter as @user1 @user2"))
                .addStringOption(option => option.setName("include").setDescription("Include users that are not in vc, enter as @user1 @user2"))
        )
        .addSubcommand(subcommand => 
            subcommand
                .setName("random")
        ...
        .addSubcommand(subcommand => 
            subcommand
                ...
                .addIntegerOption(option => option.setName("num_groups").setDescription("Number of groups").setRequired(true))
                .addStringOption(option => option.setName("exclude").setDescription("Exclude certain vc participants, enter as @user1 @user2"))
                .addStringOption(option => option.setName("include").setDescription("Include users that are not in vc, enter as @user1 @user2"))
        ),
    ...
    async execute(interaction) {
        ...
        var voice_channel = interaction.member.voice.channel;
        ...
        if (include) {
            include.trim().split(/\s+/).forEach(user => {
                var user_id = user.substr(3, 18)
                var member = interaction.member.guild.members.cache.get(user_id);
                if (member) {
                    participants.push(member.user.tag);
                }
            });
        ...
        if (exclude) {
            exclude.trim().split(/\s+/).forEach(user => {
                var user_id = user.substr(3, 18);
                var member = interaction.member.guild.members.cache.get(user_id);
                if (member) {
                    var index = participants.indexOf(member.user.tag);
                    if (index !== -1) {
