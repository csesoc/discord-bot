    ...
    async execute(interaction) {
        ...
        if (interaction.options.getSubcommand() === 'current') {
            if(!allowedChannels.some(c => c === interaction.channelId)){
                ...
                return await interaction.reply({ content: `❌ | You are not allowed to post anonymously in the channel \`${c_name[0].channel_name}\`.`, ephemeral: true });
            }
            ...
            if(channel.type != "GUILD_TEXT"){
                return await interaction.reply({ content: `Channel \`${c_name}\` not a text channel!`, ephemeral: true});
            } else if(!allowedChannels.some(c => c === c_id)){
                return await interaction.reply({ content: `❌ | You are not allowed to post anonymously in the channel \`${c_name}\`.`, ephemeral: true });
            }
            ...
            interaction.reply({ content: "Done!", ephemeral: true});
            return await interaction.guild.channels.cache.get(c_id).send(msg + '\n\n(The above message was anonymously posted by a user)');
        }
        ...
        if (interaction.options.getSubcommand() === "allow") {
            ...
            if (allowedChannels.some(c => c === channel.id)) {
                return await interaction.reply({ content: `❌ | The allowed channels list already contains \`${channel.name}\`.`, ephemeral: true });
            }
            ...
            if (!allowedChannels.some(c => c === channel.id)) {
                return await interaction.reply({ content: `❌ | The allowed channel list does not contain \`${channel.name}\`.`, ephemeral: true });
            }
